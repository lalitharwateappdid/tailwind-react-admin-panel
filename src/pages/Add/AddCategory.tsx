...
const AddCategory = () => {
    ...
    try {
      if (child == false) {
        ...
        const response = await axios.post(
          `${apiLink}category/create-parent-category`,
          {
            name: name,
        ...
        const response = await axios.post(
          `${apiLink}category/create-child-category`,
          {
            name: name,
            description: description,
            image: image.cover_image,
  ...
  const mapCategory = async () => {
    try {
      ...
      if(category_id == "" && parentCategory == ""){
        Notify("Please Select Options from both dropdown");
      }
      else{
  ...
  const fetchData = async () => {
    ...
    setChildCategoryDropdown(
      result.filter((data) => data.masterCategory == '0'),
    );
  };
  ...
  return (
    <>
      <DefaultLayout>
        <Breadcrumb
          pageName={toggleForm == false ? 'Add Category' : 'Map Category'}
        />
        <div className="p-2 flex justify-center">
          <span className="mr-2 font-semibold">Add Category</span>{' '}
          <InputSwitch
            checked={toggleForm == false ? false : true}
            onChange={(e) => setToggleForm(e.target.value)}
          />{' '}
          <span className="ml-2 font-semibold">Map Category</span>
        </div>
        {toggleForm == false ? (
